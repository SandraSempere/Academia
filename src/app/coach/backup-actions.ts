...
import { buildBackupArchive, loadBackupArchive, applyBackupArchive, isZipBuffer } from "@/lib/backup-archive";
...
export async function runBackup(formData: FormData) {
  ...
  const archiveBytes = await buildBackupArchive(dbBytes);
  const finalBytes = encrypted ? encryptBuffer(archiveBytes, passphrase) : archiveBytes;
  const filename = `copia-${timestamp()}${encrypted ? "-cifrada.enc" : ".zip"}`;
...
export async function restoreBackup(formData: FormData) {
  ...
  let payload: Buffer;
  try {
    payload = isEncrypted ? decryptBuffer(downloaded, passphrase) : downloaded;
  } catch {
    throw new Error("No se pudo descifrar la copia — revisa que la contraseña sea correcta.");
  }

  // Copias nuevas son un .zip con la base de datos + los PDFs subidos;
  // copias antiguas (de antes de que la copia incluyera los PDFs) son la
  // base de datos SQLite en crudo — se siguen aceptando las dos.
  if (isZipBuffer(payload)) {
    const { zip, dbBytes } = await loadBackupArchive(payload);

    // Verificación mínima de que la base de datos dentro del zip es real
    // antes de sustituir nada (evita dejar la app rota si la contraseña era
    // incorrecta y el "descifrado" solo dio bytes basura).
    ...
    await applyBackupArchive(zip);
  } else {
    const header = payload.subarray(0, 16).toString("utf8");
    if (!header.startsWith("SQLite format 3")) {
      throw new Error("El archivo restaurado no es una copia válida (¿contraseña incorrecta?).");
    }

    await prisma.$disconnect();
    await writeFile(dbFilePath(), payload);
  }
